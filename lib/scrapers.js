...
export async function scrapeHornbachDirect(url) {
  try {
    ...
    if (resp.ok) {
      ...
      if (price) return { price_num: parseFloat(price), price: formatPrice(parseFloat(price)), currency: '€', available: true };
    }
    
    // Fallback to page scraping with different headers
    ...
    if (priceMatch) {
      ...
      if (num > 1) return { price_num: num, price: formatPrice(num), currency: '€', available: true };
    }
